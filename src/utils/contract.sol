...
interface IFeeSharing {
    ...
    function getBalanceUpdatedBlock(address _smartContract) external view returns (uint256);
...
contract SFSFractioniser {
    ...
    event PredictionSubmitted(address indexed predictor, uint256 predictedNumber);
    event WinnersDetermined(address indexed firstPlace, address indexed secondPlace, address indexed thirdPlace);
   ...
   constructor(address _feeSharingContractAddress, uint256 _sfsTokenId) {
    ...
    function determineWinners() private {
        ...
        uint256[3] memory closestDiff = [type(uint256).max, type(uint256).max, type(uint256).max];
        ...
        emit WinnersDetermined(predictions[closest[0]].predictor, predictions[closest[1]].predictor, predictions[closest[2]].predictor);
    }
    ...
    function claimReward() external {
        ...
        if (position == 1) { // 50% for 1st place
            reward = totalReward / 2;
        } else if (position == 2) { // 30% for 2nd place
            reward = totalReward * 30 / 100;
        } else if (position == 3) { // 20% for 3rd place
    ...
    function checkRegistrationAndBalanceBlock() external view returns (bool, uint256) {
        bool isRegistered = feeSharingContract.isRegistered(address(this));
        uint256 balanceBlock = feeSharingContract.getBalanceUpdatedBlock(address(this));
